...
/**
 * Three lists and the settings, and deliberately no counts beside them: a
 * number on the rail that disagrees with the page it points at is worse
 * than no number.
 */
const NAV = [
...
export function AdminShell({
	...
	return (
		<>
			...
			>
				<Sidebar collapsible="icon">
					...
					<SidebarFooter className="border-t border-sidebar-border p-2">
						{/* At the foot rather than among the pages: it is not a place
					    things live, it is a way of reaching all of them. */}
						<SidebarMenu>
							<SidebarMenuItem>
								<SidebarMenuButton
									onClick={openAsk}
			...
			{/* Outside the provider, not only outside its grid. The provider is
			    `relative z-10`, which makes it a stacking context, and a fixed
			    element inside one cannot rise above it: sonner's own z-index was
			    being measured against the shell rather than against the sheet
			    portals on <body>, which is why every toast landed under an open
			    drawer. Sonner's wrapper is also a static <section>, so as a grid
			    child it became a row of its own, which is what once shortened the
			    sign-in page. */}
			<Toaster />
		</>
	);
